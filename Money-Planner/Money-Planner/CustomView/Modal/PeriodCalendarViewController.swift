...
class PeriodCalendarModal: UIViewController, FSCalendarDelegate, FSCalendarDataSource, FSCalendarDelegateAppearance {
    ...
    @objc func selectOneWeek() {
        endDate = nil
        if startDate != nil {
            selectDate(Calendar.current.date(byAdding: .weekOfYear, value: 1, to: startDate!)!)
        }
    }
    ...
    @objc func selectTwoWeeks() {
        endDate = nil
        if startDate != nil {
            selectDate(Calendar.current.date(byAdding: .weekOfYear, value: 2, to: startDate!)!)
        }
    }
    ...
    @objc func selectOneMonth() {
        endDate = nil
        if startDate != nil {
            selectDate(Calendar.current.date(byAdding: .month, value: 1, to: startDate!)!)
        }
    }
    ...
    private func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, titleFontFor date: Date) -> UIFont? {
        let boldFont = UIFont(name: "SFProDisplay-Bold", size: 20) ?? UIFont.boldSystemFont(ofSize: 20)
        let regularFont = UIFont(name: "SFProDisplay-Regular", size: 20) ?? UIFont.systemFont(ofSize: 20)
