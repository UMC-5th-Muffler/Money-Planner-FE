...
final class HomeRepository : BaseRepository<HomeAPI> {
    ...
    func getCalendarListWithCategory(goalId : Int, categoryId: Int, yearMonth : String, completion: @escaping (Result<HomeNowWithCategory?, BaseError>) -> Void){
        print(goalId)
        print(categoryId)
        provider.request(.getCalendarListWithCategory(goalId: goalId, categoryId: categoryId, yearMonth: yearMonth)) {
            ...
            case let .success(response):
                do {
                    let response = try response.map(BaseResponse<HomeNowWithCategory?>.self)
                    if(response.isSuccess!){
                        completion(.success(response.result!))
                    }else{
                        completion(.failure(.failure(message: response.message!)))
